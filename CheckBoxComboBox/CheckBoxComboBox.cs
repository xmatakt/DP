...
namespace PresentationControls
{
    ...
    public partial class CheckBoxComboBox : PopupComboBox
    {
            ...
            : base()
        {
            ...
            this.TextChanged += CheckBoxComboBox_TextChanged;
            this.Click += CheckBoxComboBox_Click;
        }

        private void CheckBoxComboBox_Click(object sender, EventArgs e)
        {
            ShowDropDown();
        }

        private void CheckBoxComboBox_TextChanged(object sender, EventArgs e)
        {
            StringBuilder sb = new StringBuilder();
            bool isFirst = true;
            foreach (var item in Items)
            {
                if (CheckBoxItems[item.ToString()].Checked)
                {
                    if (!isFirst)
                        sb.Append(", ");
        ...
        protected override void OnResize(EventArgs e)
        {
            ...
            Size Size = new Size(Width, height > 150 ? 150 : height);
            dropDown.Size = Size;
            base.OnResize(e);
        }
        ...
        private int getHeight()
        {
            ...
            int? count = CheckBoxItems?.Count;
            if (count != null && count > 0)
            {
                itemHeight = CheckBoxItems[0].Height + 9;
            }
            else
                return 10;

            if (itemHeight != null)
                return count.Value * itemHeight.Value;
            else
